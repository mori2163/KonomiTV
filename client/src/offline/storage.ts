...
const DB_VERSION = 3;  // サムネイル用ストアを追加したのでバージョンアップ
...
async function getDatabase(): Promise<IDBPDatabase<OfflineDBSchema>> {
    ...
    dbPromise = openDB<OfflineDBSchema>(DB_NAME, DB_VERSION, {
        upgrade(database, oldVersion) {
            ...
            // バージョン2で追加: コメント用ストア
            if (oldVersion < 2 && !database.objectStoreNames.contains('comments')) {
                database.createObjectStore('comments');
            }
            // バージョン3で追加: サムネイル用ストア
            if (oldVersion < 3 && !database.objectStoreNames.contains('thumbnails')) {
