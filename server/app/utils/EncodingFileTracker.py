...
class EncodingFileTracker:
    ...
    async def isEncodingFile(self, file_path: str | Path) -> bool:
        ...
        async with self._file_lock:
            ...
            # デバッグログ
            if is_encoding:
                logging.debug(f'File is in encoding tracker: {file_path_str}')
                logging.debug(f'All tracked files: {list(self._encoding_files)}')
    ...
    async def debugPrintTrackedFiles(self) -> None:
        ...
        async with self._file_lock:
            if self._encoding_files:
                logging.info(f'Currently tracked encoding files: {list(self._encoding_files)}')
            else:
