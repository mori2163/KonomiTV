...
`Discord.py` を利用して、Discord 連携機能を追加しています。他にも一部追加しています。詳しくはコミット履歴を見てください。自宅で運用するうえで欲しい機能を思ったまま作ったフォークなのであしからず。
*   **起動/終了通知:** KonomiTV サーバーの起動・終了時に、指定した Discord チャンネルへ通知を送信します。(注意:初期設定が必要)
*   **スラッシュコマンド:** 録画/予約番組や現在の番組情報などが表示できます。詳しくは`/help`でコマンド一覧を表示できます。
...
# 一時的な uv 仮想環境の構築 (UpdateThirdparty の実行に必要)
cd server/
uv venv --python 3.11
uv sync
...
# サードパーティーライブラリ内のスタンドアローン版 Python を明示的に指定して uv 仮想環境を再構築
...
uv venv --python /Develop/KonomiTV/server/thirdparty/Python/python.exe
# Linux
rm -rf .venv/
uv venv --python /Develop/KonomiTV/server/thirdparty/Python/bin/python
