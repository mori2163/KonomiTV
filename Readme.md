...
`Discord.py` を利用して、以下の Discord 連携機能を追加しています。
Botの導入方法については、[Wiki](https://github.com/mori2163/KonomiTV/wiki/Discord)を参照してください。
*   **起動/終了通知:** KonomiTV サーバーの起動・終了時に、指定した Discord チャンネルへ通知を送信します。(注意:初期設定が必要)
*   **スラッシュコマンド:** Discord 上から以下の操作を実行できます。
    *   `/help`: コマンド一覧を表示
    *   `/setting log_channel`: ログ出力チャンネルを設定
    *   `/setting enable_log`: ログ出力の有効/無効を切り替え
    *   `/view channel_list`: チャンネル一覧を表示 (地デジ(GR)/BS/CS/all)
    *   `/view channel_now`: 指定チャンネルの現在/次の番組情報を表示
