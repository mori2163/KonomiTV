...
- **キャプション画像ページ**
- **番組表**
- **ついで録画機能**: ストリーミング視聴時についでに録画を行う機能。チューナー枠を無駄に消費することなく同時録画が可能
- **オフライン視聴機能**: IndexedDB を使った録画番組のオフライン保存機能。HLS 形式でダウンロードし、ニコニコ実況のコメントも保存可能。
- **EPGStationに対応**: EPGStation と連携して録画予約の管理が可能
...
[リリースページ](https://github.com/rigaya/tsreplace/releases) から最新版をダウンロードして、```KonomiTV\server\thirdparty\tsreplace``` となるように配置してください。ただし、**Docker 環境で利用する場合は自動でインストールされます。**
